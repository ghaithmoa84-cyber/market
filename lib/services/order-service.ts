...
export async function createOrder(
  ...
  const cached = await findIdempotentResult(input.idempotencyKey, sessionUserId, "createOrder")
  ...
  try {
    ...
    if (isUniqueViolation(err)) {
      ...
      for (let attempt = 0; attempt < 6; attempt++) {
        const won = await findIdempotentResult(input.idempotencyKey, sessionUserId, "createOrder")
